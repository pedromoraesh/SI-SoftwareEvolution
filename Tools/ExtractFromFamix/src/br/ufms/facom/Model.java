...
public class Model {
	...
	private ArrayList<Method> listMethods;
		...
		listParameters = putTypeOnParameters(listParameters, listClasses, listTypes,listEnum, listPrimitive, listPType);
		listLocalVariables = putTypeOnLocalVariables(listLocalVariables, listClasses, listTypes,listEnum, listPrimitive, listPType);
		...
		listMethods = relationBetweenMethodAndReturnType(listMethods, listClasses, listTypes, listPrimitive, listPType, listEnum);
		...
		listClasses = recognizeTypesOfAttributes(listClasses, listTypes, listEnum, listPrimitive, listPType);
		listClasses = doInheritance(listClasses, listInheritance, listPClass);
		listClasses = mountContainers(listClasses, listPClass, listMethods, listPType, listNamespace, listTypes, listAttributes);
		fullNameForClasses(listClasses, listTypes, listPClass, listPType, listMethods, listAttributes);
		...
		listInvocations = recieverOfIvocation(listInvocations, listAttributes, listEnum, listClasses, listTypes, listPType, listPClass, listParameters);
		listInvocations = fullNameForInvocations(listInvocations);
		...
		private ArrayList<Invocation> recieverOfIvocation(ArrayList<Invocation> listInvocations,
				ArrayList<Attribute> listAttributes, ArrayList<Enumm> listEnum, ArrayList<Class> listClasses,
				ArrayList<Type> listTypes, ArrayList<ParameterizedType> listPType,
				ArrayList<ParameterizableClass> listPClass, ArrayList<Parameter> listParameters) {
			for(Invocation invocation: listInvocations){
				for(Attribute attribute: listAttributes){
					if(invocation.getRecieverID() == attribute.getId()){
						invocation.setReciever(attribute);
					}
				}
				for(Parameter parameter: listParameters){
					if(invocation.getRecieverID() == parameter.getId()){
						invocation.setReciever(parameter);
					}
				}
				for(Enumm enumm: listEnum){
					if(invocation.getRecieverID() == enumm.getId()){
						invocation.setReciever(enumm);
					}
				}
				for(Class classe: listClasses){
					if(invocation.getRecieverID() == classe.getId()){
						invocation.setReciever(classe);
					}
				}
				for(Type type: listTypes){
					if(invocation.getRecieverID() == type.getId()){
						invocation.setReciever(type);
					}
				}
				for(ParameterizedType pType: listPType){
					if(invocation.getRecieverID() == pType.getId()){
						invocation.setReciever(pType);
					}
				}
				for(ParameterizableClass pClass: listPClass){
					if(invocation.getRecieverID() == pClass.getId()){
						invocation.setReciever(pClass);
					}
				}
			}
		...
		private ArrayList<Invocation> fullNameForInvocations(ArrayList<Invocation> listInvocations) {
			for(Invocation invocation: listInvocations){
				if(invocation.getReciever() != null){
					String fullName = StringUtils.remove(invocation.getReciever().getFullName(), invocation.getReciever().getName());
					invocation.setFullName(fullName + invocation.getSignature());
				}
		...
		private void fullNameForClasses(ArrayList<Class> listClasses, ArrayList<Type> listTypes, 
			...
			for(Method method: listMethods){
				method.setFullName(method.getContainer().getFullName() + "." + method.getSignature());
				for(Parameter parameter: method.getParameters()){
					parameter.setFullName(method.getFullName() + "." + parameter.getName());
				}
			}
			for(Attribute attribute: listAttributes){
				attribute.setFullName(attribute.getContainer().getFullName() + "." + attribute.getName());
			}
			...
			for (Map.Entry<Integer, FamixEntity> entry : map.entrySet())
			{
				...
				if(StringUtils.contains(entry.getValue().getType(), "FAMIX.Method")){
					...
					for (String[] array : listAux){
						...
						else if(StringUtils.contains(array[0], "receiver")){
							invocation.setRecieverID(Integer.parseInt(array[2]));
						}
					}
